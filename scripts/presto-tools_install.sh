...
#  Version 2.0 (2025-08-21): Refactored to use systemd-journald logging. Added --verbose flag. Improved error handling for whiptail and git commands.#  Version 1.1 (2025-08-15): Added a check to ensure scripts are executable. Added bash completion for presto_drive_status.sh.
#  
#
#########################################################################################################################
...
Options:
 ...
 --interactive, -i  Interactive mode. Prompt before overwriting existing files.
...
do_configure_bash() {
  ...
  if [ -f "$ALIAS_DEST" ]; then
    if [ "$INTERACTIVE" = True ]; then
      if whiptail --yesno "The ~/.bash_aliases file already exists. Overwrite it with the presto-tools version?" 20 60; then
      ...
      log_message "INFO" "Non-interactive mode: ~/.bash_aliases already exists. Skipping overwrite."
    fi
  else
